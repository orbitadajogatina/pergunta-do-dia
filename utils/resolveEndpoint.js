...
async function resolveEndpoint(req, res, endpoint, method, apiAuthorization) {
  ...
  if (!authorization)
    return res
      .status(401)
      .send(
        "`Invalid token. Please get your API key on Discord using the bot or through the website."
      );
  if (authorization === "suspended")
  ...
  try {
    ...
    res
      .status(404)
      .send(
        `${method.toUpperCase()} method not available for this endpoint: ${param}`
      );
  } catch (error) {
