...
async function apiAuthorization(token) {
    ...
    const { data } = await database.from('api').select().eq('token', token).single();
...
async function getAdmins(bot) {
    ...
    return ['668199172276748328', ...members.filter(m => m.roles.cache.has(process.env.ADMIN_ROLE_ID)).map(m => m.id)];
}
